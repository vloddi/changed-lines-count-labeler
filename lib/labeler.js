...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const changedLinesCnt = config.exclude.length > 0
                ? yield getChangedLinesCountExcluding(client, prNumber, config.exclude)
...
function getChangedLinesCountExcluding(client, prNumber, exclude) {
    return __awaiter(this, void 0, void 0, function* () {
        const matchers = exclude.map((pattern) => new minimatch_1.Minimatch(pattern, { dot: true }));
        const files = yield client.paginate(client.rest.pulls.listFiles, {
            owner: github.context.repo.owner,
            repo: github.context.repo.repo,
            pull_number: prNumber,
            per_page: 100,
...
function getConfigFromObject(configObject) {
    const labels = new Map();
    let exclude = [];
    for (const key in configObject) {
        if (key === "exclude") {
            if (!Array.isArray(configObject[key]) || configObject[key].some((p) => typeof p !== "string")) {
